...
class CodeAnalyzer implements Opcode
{
    ...
    private void visitBytecode(final CodeIterator ci, final int[] stack, int index) throws BadBytecode {
        ...
        while (ci.hasNext()) {
            ...
            if (this.processBranch(op, ci, index, codeLength, stack, stackDepth)) {
                break;
            }
            if (isEnd(op)) {
                break;
    ...
    private boolean processBranch(final int opcode, final CodeIterator ci, final int index, final int codeLength, final int[] stack, final int stackDepth) throws BadBytecode {
        if ((153 <= opcode && opcode <= 166) || opcode == 198 || opcode == 199) {
            ...
            switch (opcode) {
                ...
                case 201: {
                    ...
                    throw new BadBytecode("sorry, cannot compute this data flow due to JSR");
                }
                case 169: {
                    if (stackDepth == 1) {
                        return true;
                    }
                    throw new BadBytecode("sorry, cannot compute this data flow due to RET");
                }
                case 170:
                case 171: {
