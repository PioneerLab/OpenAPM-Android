...
public class BasicBlock
{
    ...
    public static class Maker
    {
            ...
            while (ci.hasNext()) {
                ...
                if ((153 <= op && op <= 166) || op == 198 || op == 199) {
                    ...
                    switch (op) {
                        case 167:
                        case 168: {
                            this.makeGotoJsr(marks, index, index + ci.s16bitAt(index + 1), op == 167, 3);
                            continue;
                        }
                        case 169: {
                    ...
                    this.makeGotoJsr(marks, index, index + ci.s32bitAt(index + 1), op == 200, 5);
                }
                else {
        ...
        private void makeGotoJsr(final HashMap marks, final int pos, final int target, final boolean isGoto, final int size) {
            final Mark to = this.makeMark(marks, target);
            BasicBlock[] jumps;
            if (isGoto) {
                jumps = this.makeArray(to.block);
            }
            else {
                final Mark next = this.makeMark(marks, pos + size);
                jumps = this.makeArray(to.block, next.block);
            }
            this.makeMark(marks, pos, jumps, size, isGoto);
        }
            ...
            while (i < marks.length) {
                ...
                if (bb == null) {
                    ...
                    if (prev.length == 0) {
                        ...
                        final int prevPos = prev.position;
