...
public abstract class Tracer implements TypeTag
{
    ...
    public Tracer(final Tracer t, final boolean copyStack) {
        this.classPool = t.classPool;
        this.cpool = t.cpool;
        this.returnType = t.returnType;
        this.stackTop = t.stackTop;
        final int size = t.stackTypes.length;
        this.stackTypes = new TypeData[size];
        if (copyStack) {
            copyFrom(t.stackTop, t.stackTypes, this.stackTypes);
        }
        final int size2 = t.localsTypes.length;
        this.localsTypes = new TypeData[size2];
        copyFrom(size2, t.localsTypes, this.localsTypes);
    }
    
    protected static int copyFrom(final int n, final TypeData[] srcTypes, final TypeData[] destTypes) {
        int k = -1;
        for (int i = 0; i < n; ++i) {
            final TypeData t = srcTypes[i];
            destTypes[i] = ((t == Tracer.TOP) ? Tracer.TOP : t.getSelf());
            if (t != Tracer.TOP) {
                if (t.is2WordType()) {
                    k = i + 1;
                }
    ...
    private void throwBadBytecode(final int pos, final String name) throws BadBytecode {
    ...
    private int doOpcode54_95(final int pos, final byte[] code, final int op) throws BadBytecode {
        final TypeData[] localsTypes = this.localsTypes;
        final TypeData[] stackTypes = this.stackTypes;
        switch (op) {
            case 54: {
            ...
            case 62: {
                final int var = op - 59;
                localsTypes[var] = Tracer.INTEGER;
                --this.stackTop;
                break;
            }
            case 63:
            ...
            case 66: {
                final int var = op - 63;
                localsTypes[var] = Tracer.LONG;
                localsTypes[var + 1] = Tracer.TOP;
                this.stackTop -= 2;
                break;
            }
            case 67:
            ...
            case 83: {
                TypeData.setType(stackTypes[this.stackTop - 1], TypeData.ArrayElement.getElementType(stackTypes[this.stackTop - 3].getName()), this.classPool);
                this.stackTop -= 3;
                break;
            }
            case 84:
            ...
            case 91: {
                ...
                stackTypes[sp2 - len] = stackTypes[sp2];
                this.stackTop = sp2 + 1;
                break;
            }
            case 92: {
            ...
            case 94: {
                ...
                final int sp2 = this.stackTop;
                stackTypes[sp2 - len] = stackTypes[sp2];
                stackTypes[sp2 - len + 1] = stackTypes[sp2 + 1];
                this.stackTop = sp2 + 2;
                break;
            }
            case 95: {
                final int sp = this.stackTop - 1;
                final TypeData t = stackTypes[sp];
                stackTypes[sp] = stackTypes[sp - 1];
                stackTypes[sp - 1] = t;
                break;
            }
    ...
    private int doOpcode96_147(final int pos, final byte[] code, final int op) {
        ...
        switch (op) {
            ...
            case 133: {
                this.stackTypes[this.stackTop] = Tracer.LONG;
                this.stackTypes[this.stackTop - 1] = Tracer.TOP;
                ++this.stackTop;
                break;
            }
            case 134: {
            ...
            case 135: {
                this.stackTypes[this.stackTop] = Tracer.DOUBLE;
                this.stackTypes[this.stackTop - 1] = Tracer.TOP;
                ++this.stackTop;
                break;
            }
    ...
    private int doOpcode148_201(final int pos, final byte[] code, final int op) throws BadBytecode {
        switch (op) {
            ...
            case 176: {
                ...
                final int stackTop2 = this.stackTop - 1;
                this.stackTop = stackTop2;
                TypeData.setType(stackTypes2[stackTop2], this.returnType, this.classPool);
                this.visitReturn(pos, code);
                break;
            }
            case 177: {
            ...
            case 191: {
                ...
                final int stackTop3 = this.stackTop - 1;
                this.stackTop = stackTop3;
                TypeData.setType(stackTypes3[stackTop3], "java.lang.Throwable", this.classPool);
    ...
    private int doPutField(final int pos, final byte[] code, final boolean notStatic) throws BadBytecode {
        ...
        if (c == 'L') {
            TypeData.setType(this.stackTypes[this.stackTop], getFieldClassName(desc, 0), this.classPool);
        }
        else if (c == '[') {
    ...
    private void setFieldTarget(final boolean notStatic, final int index) throws BadBytecode {
        if (notStatic) {
            ...
            final int stackTop = this.stackTop - 1;
            this.stackTop = stackTop;
            TypeData.setType(stackTypes[stackTop], className, this.classPool);
        }
    }
