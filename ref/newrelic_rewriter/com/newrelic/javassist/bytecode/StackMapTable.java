...
public class StackMapTable extends AttributeInfo
{
    ...
    static class Copier extends SimpleCopy
    {
        ...
        public Copier(final ConstPool src, final byte[] data, final ConstPool dest) {
            super(data);
            this.srcPool = src;
            this.destPool = dest;
        }
        ...
        protected int copyData(final int tag, final int data) {
            if (tag == 7) {
                return this.srcPool.copy(data, this.destPool, null);
            }
            return data;
        }
        ...
        protected int[] copyData(final int[] tags, final int[] data) {
            ...
            for (int i = 0; i < data.length; ++i) {
                if (tags[i] == 7) {
                    newData[i] = this.srcPool.copy(data[i], this.destPool, null);
                }
                else {
                    newData[i] = data[i];
                }
    ...
    static class Shifter extends Walker
    {
        ...
        private void update(final int pos, final int offsetDelta, final int base, final int entry) {
        ...
        private static byte[] insertGap(final byte[] info, final int where, final int gap) {
        ...
        private void update(final int pos, final int offsetDelta) {
            final int oldPos = this.position;
            this.position = oldPos + offsetDelta + ((oldPos != 0) ? 1 : 0);
            boolean match;
            if (this.exclusive) {
