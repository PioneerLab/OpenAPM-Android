...
class CtClassType extends CtClass
{
    ...
    public CtClass[] getNestedClasses() throws NotFoundException {
        ...
        for (int i = 0; i < n; ++i) {
            final String outer = ica.outerClass(i);
            if (outer == null || outer.equals(thisName)) {
                final String inner = ica.innerClass(i);
                if (inner != null) {
    ...
    public CtField getField(final String name) throws NotFoundException {
        final CtField f = this.getField2(name);
        if (f == null) {
    ...
    CtField getField2(final String name) {
        final CtField df = this.getDeclaredField2(name);
        if (df != null) {
            return df;
        }
        try {
            ...
            for (int num = ifs.length, i = 0; i < num; ++i) {
                final CtField f = ifs[i].getField2(name);
                if (f != null) {
                    return f;
                }
            }
    ...
    public CtField getDeclaredField(final String name) throws NotFoundException {
        final CtField f = this.getDeclaredField2(name);
        if (f == null) {
            throw new NotFoundException("field: " + name + " in " + this.getName());
        }
        return f;
    ...
    private CtField getDeclaredField2(final String name) {
        final CtMember.Cache memCache = this.getMembers();
        CtMember field = memCache.fieldHead();
        final CtMember tail = memCache.lastField();
        while (field != tail) {
