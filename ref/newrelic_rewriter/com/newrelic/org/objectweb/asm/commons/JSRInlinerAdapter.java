...
import com.newrelic.org.objectweb.asm.tree.LocalVariableNode;
import com.newrelic.org.objectweb.asm.tree.InsnNode;
import java.util.List;
import java.util.ArrayList;
import com.newrelic.org.objectweb.asm.tree.InsnList;
import java.util.LinkedList;
import com.newrelic.org.objectweb.asm.tree.LookupSwitchInsnNode;
import com.newrelic.org.objectweb.asm.tree.TableSwitchInsnNode;
...
import com.newrelic.org.objectweb.asm.tree.LabelNode;
import com.newrelic.org.objectweb.asm.tree.JumpInsnNode;
...
import com.newrelic.org.objectweb.asm.tree.MethodNode;
...
public class JSRInlinerAdapter extends MethodNode implements Opcodes
{
    ...
    private void markSubroutineWalk(final BitSet set, final int n, final BitSet set2) {
        ...
        while (i != 0) {
            ...
            for (final TryCatchBlockNode tryCatchBlockNode : this.tryCatchBlocks) {
                final int index = this.instructions.indexOf((AbstractInsnNode)tryCatchBlockNode.handler);
                if (set.get(index)) {
                    continue;
                }
                final int index2 = this.instructions.indexOf((AbstractInsnNode)tryCatchBlockNode.start);
                final int index3 = this.instructions.indexOf((AbstractInsnNode)tryCatchBlockNode.end);
    ...
    private void markSubroutineWalkDFS(final BitSet set, int n, final BitSet set2) {
        while (true) {
            ...
            if (value.getType() == 7 && value.getOpcode() != 168) {
                this.markSubroutineWalkDFS(set, this.instructions.indexOf((AbstractInsnNode)((JumpInsnNode)value).label), set2);
            }
            if (value.getType() == 11) {
                final TableSwitchInsnNode tableSwitchInsnNode = (TableSwitchInsnNode)value;
                this.markSubroutineWalkDFS(set, this.instructions.indexOf((AbstractInsnNode)tableSwitchInsnNode.dflt), set2);
            ...
            if (value.getType() == 12) {
                ...
                this.markSubroutineWalkDFS(set, this.instructions.indexOf((AbstractInsnNode)lookupSwitchInsnNode.dflt), set2);
    ...
    private void emitSubroutine(final JSRInlinerAdapter$Instantiation jsrInlinerAdapter$Instantiation, final List list, final InsnList list2, final List list3, final List list4) {
        ...
        for (int i = 0; i < this.instructions.size(); ++i) {
            ...
            if (value.getType() == 8) {
                ...
                if (rangeLabel != o) {
                    list2.add((AbstractInsnNode)rangeLabel);
                    o = rangeLabel;
                }
            }
                ...
                if (value.getOpcode() == 169) {
                    ...
                    list2.add((AbstractInsnNode)new JumpInsnNode(167, returnLabel));
                }
                    ...
                    list2.add((AbstractInsnNode)new InsnNode(1));
                    list2.add((AbstractInsnNode)new JumpInsnNode(167, gotoLabel));
                    list2.add((AbstractInsnNode)jsrInlinerAdapter$Instantiation2.returnLabel);
                    list.add(jsrInlinerAdapter$Instantiation2);
                }
                else {
                    list2.add(value.clone((Map)jsrInlinerAdapter$Instantiation));
                }
            }
        }
        for (final TryCatchBlockNode tryCatchBlockNode : this.tryCatchBlocks) {
    ...
    static {
        JSRInlinerAdapter.class$org$objectweb$asm$commons$JSRInlinerAdapter = class$("com.newrelic.org.objectweb.asm.commons.JSRInlinerAdapter");
    }
}
